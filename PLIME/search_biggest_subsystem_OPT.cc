...
//	  / sqrt(1 + x1[1]*x1[1] + x1[2]*x1[2]);
	...
	if (P_ALGO->no_corrections == 0 || ABS(dist) > P_ALGO->epsilon) {
	  if (dist < 0 ) { /* dist<0 & auto_temp */
	    x1[0]+= (ti/t0) * A[i][0] * exp(-ABS(dist)/ti)
	      * ALGO->weight[0];
	    x1[1]+= (ti/t0) * A[i][1] * exp(-ABS(dist)/ti)
	      * ALGO->weight[1];
	    x1[2]+= (ti/t0) * A[i][2] * exp(-ABS(dist)/ti)
	      * ALGO->weight[2];
	  }
...
//      }
    ...
    for (i = 0; i < ALGO->nb_couples; i++) {
      k=eq[i];
      if (ABS((qprod_scal(x1, A[k], nc) - b[k])/b[k])
