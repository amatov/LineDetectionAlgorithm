			   ...
			   point_2d_float u)
{
  ...
  int y=pt.y;
  int out=1;
  for (int i=-(int)LDT->seg_epsilon; i<=(int)LDT->seg_epsilon; i++) {
//    x = FL2INT(pt.x - i*u.y);
//    y = FL2INT(pt.y + i*u.x);
    if (u.x < u.y) x = pt.x +i;
    else y = pt.y+i;
			   ...
			   struct_ldetect *LDT)
{
  ...
  if (LDT->segment) { /* segment analysis */
    ...
    int i,i1,i2,tot_pts,flag;
    int segment_start, segment_length;

    /* current solution in the absolute coord system */
    float abs_a,abs_b,abs_c;
    abs_a = x[0];
    abs_b = x[1];
    abs_c = x[2] + x[0]*(0.5 - LDT->image.dim.x/2) 
      + x[1]*(0.5 - LDT->image.dim.y/2);

    /* compute direction vector */
    float norm_u = sqrt(abs_a*abs_a + abs_b*abs_b);
    if (abs_b<0) {
      u.x = -abs_b/norm_u;
      u.y =  abs_a/norm_u;
    }
    ...
    /* start_pt computing */
    start_pt.y = (int)(-(abs_c+abs_a*X_MIN)/abs_b);
    if (start_pt.y>=Y_MIN && start_pt.y<Y_MAX)
      /* left cluster edge */
      start_pt.x = X_MIN;
    else {
      start_pt.x = (int)(-(abs_c+abs_b*Y_MIN)/abs_a);
      if (start_pt.x>=X_MIN && start_pt.x<=-(abs_c+abs_b*Y_MAX)/abs_a)
	/* upper cluster edge */
	start_pt.y = Y_MIN;
      else { 
	/* bottom cluster edge */
	start_pt.x = (int)(-(abs_c+abs_b*Y_MAX)/abs_a);
	start_pt.y = Y_MAX;
      }
    }

    current_pt0.x = start_pt.x;
    current_pt0.y = start_pt.y;
    i = i1 = i2 = 0;
    segment_length = tot_pts = 0;
//    int nb_pts = nb_pts_2epsilon(LDT, current_pt0, u);
    int nb_pts = 0;
    flag = 0;
...
//      ,current_pt0.x,current_pt0.y,tot_pts,nb_pts,i1,i2,segment_length);
      ...
      nb_pts = nb_pts_2epsilon(LDT, current_pt0, u);
    }
    ...
    mid_segment.x -= (int)((float)LDT->image.dim.x/2 - 0.5);
    mid_segment.y -= (int)((float)LDT->image.dim.y/2 - 0.5);
    float aux = -(u.x*mid_segment.x + u.y*mid_segment.y);
    ...
    for (i = 0; i < ALGO->nb_couples; i++) {
      ...
      /* pt_line_distance = |ax+by+c|/sqrt(a^2+b^2) */
      pt_line_distance = qprod_scal(x, LDT->SYST->A[k], LDT->SYST->nc)
	/ norm_ab;
      /* pt to median line of segment corresponding to the current solution */
      pt_med_line_distance = u.x*LDT->SYST->A[k][0]+u.y*LDT->SYST->A[k][1]
	+ aux;
      if (ABS(pt_line_distance) <= P_ALGO->epsilon
	  && ABS(pt_med_line_distance) <= segment_length/2+P_ALGO->epsilon)
	nb_point_fitted++;
    }

//printf(" nb_point_fitted %d | %d %d %d %d\n",nb_point_fitted
//       ,LDT->segments[ALGO->step-1][0]
//       ,LDT->segments[ALGO->step-1][1]
//       ,LDT->segments[ALGO->step-1][2]
//       ,LDT->segments[ALGO->step-1][3]);
	...
	pt_line_distance = qprod_scal(x, LDT->SYST->A[k], LDT->SYST->nc)
	  / norm_ab;
	/* pt to median line of segment corresponding to the curr. solution */
	pt_med_line_distance = u.x*LDT->SYST->A[k][0]+u.y*LDT->SYST->A[k][1]
	  + aux;
	...
	if (ABS(pt_line_distance) <= P_ALGO->epsilon
	  ...
	  if (P_ALGO->graph && P_ALGO->mon_well_classified) {
	    current_pt.x = LDT->SYST->A[k][0] + LDT->image.dim.x/2;
	    current_pt.y = LDT->SYST->A[k][1] + LDT->image.dim.y/2;
		       ...
		       struct_cdetect *CDT, float eps_threshold)
{
  int j = 0;
  int k;
  int new_nb_couples = ALGO->nb_couples;
  ...
  if (CDT->postprocess_mode==0) { /* normal postprocessing */
    for (int i = 0; i < ALGO->nb_couples; i++) {
      k=ALGO->equations[i];
      if (ABS(qprod_scal(x, A[k],CDT->SYST->nc)) < eps_threshold) {
	new_nb_couples--;
	/* update canvas */
	if (P_ALGO->graph && P_ALGO->mon_well_classified)
	  conic_detect_tk_flag_classified(CDT->TK,
					  &CDT->SYST->A[k][3], ALGO->step);
      }
      else
    ...
    exit_with_error("enhanced postprocessing not yet implemented");
  }

  /* update the nb of couples left considering the subsystem size */
  ALGO->nb_couples = new_nb_couples;
printf("new nb couple %d\n",ALGO->nb_couples);
}
#undef C_GEO_DIST
			...
			struct_cdetect *CDT, float eps_threshold)
{
  ...
  if (CDT->postprocess_mode==0) { /* normal postprocessing */
    ...
    ALGO->last_subsystem_enhanced_size = 0;
  }
	...
	if (find_tangente_of_edge(CDT->image, CDT->SYST->A[k],
	  ...
	  if (CDT->postprocess_mode == 1) {
	    /* ******** mode 1 */
	    /* enhanced size is computed but pts are removed according simple */
	    /* postprocessing scheme                                          */
	    if (ABS(delta_r) < eps_threshold) { /* pt fitted by simple postp. */
	      /* remove pt */
	      new_nb_couples--;
	      /* update canvas */
	      if (P_ALGO->graph && P_ALGO->mon_well_classified) {
		...
		ALGO->last_subsystem_enhanced_size++;
	      }
	    }
	    ...
	    /* ******** mode 2 */
	    /* enhanced size is computed and only the pts fitted by enhanced  */
	    /* postprocessing are removed                                     */
	    if (ABS(delta_r) < eps_threshold) { /* pt fitted by simple postp. */
	      /* angle calculation ((-u.y;u.x) is the direction vect. of normal) */
	      cos = ABS(-pf1_x*CDT->u.y + pf1_y*CDT->u.x)/d1;
	      if (cos > ANGULAR_THRESHOLD_CIRCLE
      ...
      if (ALGO->last_subsystem_enhanced_size
	  > ALGO->last_subsys_size*ENHANCED_THRESHOLD_CIRCLE) {
	/* subsystem accepted */
	free(ALGO->equations);
	ALGO->equations = temp_eq;
      }
      else { /* subsystem rejected */
	printf("Rejected by enhanced postprocessing (%d|%d) %4.1f%%\n",
	       ALGO->last_subsystem_enhanced_size, ALGO->last_subsys_size,
	       (float)ALGO->last_subsystem_enhanced_size
	       /(float)ALGO->last_subsys_size*100);
	new_nb_couples = ALGO->nb_couples;
	free(temp_eq);
	ALGO->last_subsystem_accepted = 0;
	...
	if (find_tangente_of_edge(CDT->image, CDT->SYST->A[k],
	  ...
	  if (ABS(delta_r) < eps_threshold) {
		...
		if (P_ALGO->graph && P_ALGO->mon_well_classified) {
	      ...
	      if (CDT->postprocess_mode == 2) temp_eq[j++] = ALGO->equations[i];
	  }
	  else /* delta_r > eps_threshold => pt rejected */
	    temp_eq[j++] = ALGO->equations[i];
	}
      ...
      if (ALGO->last_subsystem_enhanced_size
	  > ALGO->last_subsys_size*ENHANCED_THRESHOLD_ELLIPSE)
	{
	  /* subsystem accepted */
	  free(ALGO->equations);
	  ALGO->equations = temp_eq;
