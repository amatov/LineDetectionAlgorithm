...
/****************************************************************************/
void calculate_subsystem_size_cdt2(struct_algo_param *P_ALGO, struct_algo *ALGO,
  ...
  for (int i = 0; i < ALGO->nb_couples; i++) {
    int k = EQ[i];
    float d_x = CDT->SYST->A[k][0]-x[0];
    float d_y = CDT->SYST->A[k][1]-x[1];
    if (ABS(sqrt(d_x*d_x + d_y*d_y)-x[2]) < eps_threshold)
      ALGO->last_subsys_size++;
  }
  if (x[2]-eps_threshold>0) {
    int temp=0;
    for (int R=(int)(x[2]-eps_threshold); R<=(int)(x[2]+eps_threshold); R++) {
      for (float i=0; i<=TWO_PI; i+=1/(float)R) {
	int c = (int)(x[0]+R*cos(i));
	int l = (int)(x[1]+R*sin(i));
...
int rien;
    ...
    /* initialisation for computation time */
    float aux_time = get_process_time();
          ...
          if (dist_AB > MIN_AB && dist_AB < MAX_AB){
            ...
            if (ABS(ALPHA_DIST_CIRC*delta_r/ti) < P_ALGO->stop_temp) {
              ...
              if (P_ALGO->graph && iteration%MONITORING_STEP==0 && P_ALGO->mon==-1)
                conic2_detect_tk_update_mon_canvas(CDT, x, i1, i2, ALGO->ti);
              nb_corr++;
            }
          }
          else {
...
/************************************************************************/
    ...
    P_ALGO->BENCH->computation_time += get_process_time()-aux_time;

    /* calculate subsystem size */
    calculate_subsystem_size_cdt2(P_ALGO, ALGO, CDT, x);
    ...
    /* add current solution to the matrix of solutions */
     ...
     ALGO->last_subsystem_typical_size = (int)(TWO_PI*x[2]*visible_circle);
	 ...
	 (ALGO->last_subsys_size_data >= P_ALGO->stop_cycles
	  ...
	  && ALGO->last_subsys_size > ALGO->last_subsys_size_data/10)) {
       ...
       c2detect_update_eq(P_ALGO, ALGO, CDT,
			  max_value(P_ALGO->relative_eps*x[2], P_ALGO->epsilon));
     }
